...
dat <- read.csv("Database_Paper_1_20250928.csv", header=T, sep=";", dec=".")
meta <- read.csv("Database_Metadata_20250928.csv", header=T, sep=";")
...
##############################################
##############################################
## 1. PREPARATION OF FIGURE 1 = MAP
##############################################
##############################################

mapWorld <- annotation_borders("world", colour="gray50", fill="white", ylim=c(-60,70))

mp <- ggplot() + mapWorld
colors <- c("#A0082B", "#8A8C8A","#A684BE","#D59404","#027017" ,"#060D89","#99CCF6","#94F2D1","#C4657A","#B8B208")

global <- mp +
  geom_point(data = dat, aes(x = Long, y = Lat, color = Sport, shape=Gender), size=4,alpha = 0.6)+
  theme_classic()+
...
# Europe only
mapeurope <- borders("world", colour="gray50", fill="white", xlim=c(-10,20), ylim=c(38,72))

meur <- ggplot() + mapeurope
meur

dataeurope <- subset(dat, Long>-10 & Long<25 & Lat>38 & Lat<72) # 64 wildlife species

colors <- c("#8A8C8A","#A684BE","#D59404","#027017" ,"#060D89","#99CCF6","#94F2D1","#C4657A","#B8B208")
europe <- meur +
  geom_point(data = dataeurope, aes(x = Long, y = Lat, color = Sport, shape=Gender), size=4, alpha = 0.6)+
  theme_classic()+
  scale_shape_manual(values=c(16, 17))+
  scale_color_manual(values=colors)+
  theme(axis.text.x = element_text(size = 15),
...
##############################################
##############################################
## 2. PRELIMINARY DESCRIPTION OF THE DATA 
##############################################
##############################################
    ...
    nteams <- length(unique(dat$Club)) #833 clubs in total in the dataset (but not entirely informative because pools both men and women entities)
    nteams
    length(unique(dat$Club[which(dat$Gender=="Men")])) #681 post-correction
    length(unique(dat$Club[which(dat$Gender=="Women")])) #174
    # so total number of clubs (counting for both women and men entities) = 681+174 = 855
    
    
    # Number of unique taxons
    sum(table(dat$Classification_General)) #877 (but accounts for domestic species)
    
    # Number of teams that have a wild animal as a representation
    ndom <- length(which(dat$Trophic != "Domestic")) # 745 occurrences of wild animal (note that "occurrences" are different from "teams": some teams have several animals depicted in the logo)
    ndom
    nmendom <- length(unique(dat$Club[which(dat$Gender=="Men" & dat$Trophic != "Domestic")])) # 578 Men's teams have a wild animal
    nmendom 
    nwomdom <- length(unique(dat$Club[which(dat$Gender=="Women" & dat$Trophic != "Domestic")])) # 149 Women's teams have a wild animal
    nwomdom
    nmendom + nwomdom# Number of wildlife teams: 727 in total
    ...
    length(which(dat$Classification_General=="Species" & dat$Trophic!="Domestic")) # 444 occurrences 
    length(which(dat$Classification_General=="Species" & dat$Gender=="Men" & dat$Trophic!="Domestic")) #352 men's occurrences
...
Fig_S3 <- ggplot(meta, aes(y=Percent2, x=Sport, colour=Sport)) + 
...
length(unique(dat$Classification_Specific))
...
colnames(datspecies) <- c("Classification_Species","Freq","id") # 86 species in total
...
length(which(datforms$Freq==1)) # 80 teams are the only ones to use a unique animal taxon as an emblem
...
Fig_2 <- grid.arrange(plot_class,plot_species_15, nrow = 1)


##############################################
##############################################
## 4. PREPARATION OF SUPPLEMENTARY TABLE
##############################################
##############################################
...
##############################################################
##############################################################
## 5. PREPARATION OF FIGURE S4 (=differences across genders)
##############################################################
##############################################################
...
Fig_S4 <- grid.arrange(panel_d_relative,panel_c, nrow = 1)
Fig_S4

#####################################################
#####################################################
## 6. PREPARATION OF FIGURE 3 (=biplot of countries)
#####################################################
#####################################################
...
Fig_3 <- ggplot(datcountry, aes(x=nsp4, y=relative, size=NWildTeams, label=country)) + 
...
##########################################################################################
##########################################################################################
## 7. PREPARATION OF TABLE S2 and Fig S1
## Looking at relationship between taxonomic richness and local (endemic) animal richness
##########################################################################################
##########################################################################################
div_country <- read.csv("Tab6a_IUCN.csv", sep=";", header=T)

# Merge the two datasets by country name
merged_diversity <- merge(datcountry, 
                          div_country, 
                          by.x = "country", 
                          by.y = "Name")

## use ISO3 country codes
countrycode(sourcevar=merged_diversity$country, origin='country.name', destination='iso3c')
merged_diversity$cntry.code <- countrycode(sourcevar=merged_diversity$country, origin='country.name', destination='iso3c')
head(merged_diversity)

# Scatterplot with regression line
Fig_S1 <- ggplot(merged_diversity, aes(x = Total, y = nsp4, label=cntry.code)) +
  geom_point(aes(color=continentcountry), size=5) +                     # points
  scale_x_log10() +
  scale_y_log10() +
  ...
  geom_label_repel(aes(label = cntry.code),
                   ...
                   alpha=1,
                   max.overlaps = 3) +
  #geom_text_repel(size=3)+# regression line with CI
  labs(
    x = "richness of animal species",
    y = "taxonomic richness in sport emblems"
  ) +
  theme_classic()+
  theme(
    legend.position = "none"
...
merged_diversity$lnsp4.sc <- scale(log10(merged_diversity$nsp4), scale=T, center=T)
merged_diversity$ltotal.sc <- scale(log10(merged_diversity$Total), scale=T, center=T)
merged_diversity$lnwildteams.sc <- scale(log10(merged_diversity$NWildTeams), scale=T, center=T)
head(merged_diversity)

# run a simple linear regression
model_div_1.1 <- lm(lnsp4.sc ~ ltotal.sc + lnwildteams.sc, data = merged_diversity)
summary(model_div_1.1)
library(performance) # model diagnostics
check_model(model_div_1.1)
library(sjPlot) # model diagnostics
plot_model(model_div_1.1)
model_div_1.2 <- lm(lnsp4.sc ~ ltotal.sc, data = merged_diversity)
model_div_1.3 <- lm(lnsp4.sc ~ lnwildteams.sc, data = merged_diversity)
model_div_1.4 <- lm(lnsp4.sc ~ 1, data = merged_diversity)
...
LL.vals <- c(as.numeric(logLik(model_div_1.1)), as.numeric(logLik(model_div_1.2)), 
             as.numeric(logLik(model_div_1.3)), as.numeric(logLik(model_div_1.4)))
AICc.vals <- AICc(model_div_1.1, model_div_1.2, model_div_1.3, model_div_1.4)
delta.AICc.vals <- delta.AIC(AICc.vals)
weight.AICc.vals <- weight.AIC(delta.AICc.vals)
R2adj.vals <- c(summary(model_div_1.1)$adj.r.squared, summary(model_div_1.2)$adj.r.squared, 
                summary(model_div_1.3)$adj.r.squared, summary(model_div_1.4)$adj.r.squared)
mod.labs <- c("full","richness","teams","intercept")
k.vals <- c(3,2,2,1)

# summary table
model.summary <- data.frame(model=mod.labs, k=k.vals, LL=round(LL.vals,3), AICc=round(AICc.vals,3), 
                            ΔAICc=round(delta.AICc.vals,3), wAICc=round(weight.AICc.vals,3),
                            R2adj=round(R2adj.vals,3))
# sort descending on wAICc
model.summary2 <- model.summary[order(-model.summary$wAICc),]
model.summary2

## use species in emblems that are found in the country in question (replaces nsp4)
## import data
localemblems <- read.csv("localemblems.csv", header=T)
head(localemblems)
colnames(localemblems)[2] <- "country"
head(localemblems)

# merge with merged_diversity
head(merged_diversity)
merged_diversity2 <- merge(merged_diversity, localemblems, by="country", all.x=TRUE)
head(merged_diversity2)
plot(merged_diversity2$nsp4, merged_diversity2$endem, pch=19)

## remove 0s
merged_diversity2 <- merged_diversity2[-which(merged_diversity2$endem==0),]
merged_diversity2

# scale endem
merged_diversity2$lendem.sc <- scale(log10(merged_diversity2$endem), scale=T, center=T)
merged_diversity2

# Scatterplot with regression line
Fig_S1b <- ggplot(merged_diversity2, aes(x = Total, y = endem, label=cntry.code)) +
  geom_point(aes(color=continentcountry), size=5) +                     # points
  scale_x_log10() +
  scale_y_log10() +
  geom_smooth(method = "lm", se = TRUE, color = "darkred") +
  ...
  labs(
    x = "richness of animal species",
    y = "endemic taxonomic richness in sport emblems"
  ) +
  theme_classic()+
  theme(
    legend.position = "none"
...
## run a simple linear regression
model_div_2.1 <- lm(lendem.sc ~ ltotal.sc + lnwildteams.sc, data = merged_diversity2)
summary(model_div_2.1)
check_model(model_div_2.1)
plot_model(model_div_2.1)
model_div_2.2 <- lm(lendem.sc ~ ltotal.sc, data = merged_diversity2)
model_div_2.3 <- lm(lendem.sc ~ lnwildteams.sc, data = merged_diversity2)
model_div_2.4 <- lm(lendem.sc ~ 1, data = merged_diversity2)

LL.vals <- c(as.numeric(logLik(model_div_2.1)), as.numeric(logLik(model_div_2.2)), 
             as.numeric(logLik(model_div_2.3)), as.numeric(logLik(model_div_2.4)))
AICc.vals <- AICc(model_div_2.1, model_div_2.2, model_div_2.3, model_div_2.4)
delta.AICc.vals <- delta.AIC(AICc.vals)
weight.AICc.vals <- weight.AIC(delta.AICc.vals)
R2adj.vals <- c(summary(model_div_2.1)$adj.r.squared, summary(model_div_2.2)$adj.r.squared, 
                summary(model_div_2.3)$adj.r.squared, summary(model_div_2.4)$adj.r.squared)
mod.labs <- c("full","richness","teams","intercept")
k.vals <- c(3,2,2,1)

# summary table
model.summary.2 <- data.frame(model=mod.labs, k=k.vals, LL=round(LL.vals,3), AICc=round(AICc.vals,3), 
                              ΔAICc=round(delta.AICc.vals,3), wAICc=round(weight.AICc.vals,3),
                              R2adj=round(R2adj.vals,3))
# sort descending on wAICc
model.summary2.2 <- model.summary.2[order(-model.summary.2$wAICc),]
model.summary2.2


## IUCN endemic species lists
endemSR <- read.csv("Tab8a_IUCN.csv", header=T)
head(endemSR)

endemSR$country[19] <- "Cote d'Ivoire"
endemSR$country[43] <- "Reunion"
endemSR$country[174] <- "Curacao"
endemSR$country[184] <- "Saint Barthelemy"

countrycode(sourcevar=endemSR$country, origin='country.name', destination='iso3c')
endemSR$cntry.code <- countrycode(sourcevar=endemSR$country, origin='country.name', destination='iso3c')
head(endemSR)

## which cntry.code == NA
which(is.na(endemSR$cntry.code)==TRUE)
endemSR[187,]
endemSR[218,]

# merge with merged_diversity2
head(merged_diversity2)
merged_diversity3 <- merge(merged_diversity2, endemSR, by="cntry.code", all.x=TRUE)
head(merged_diversity3)

## calculate endemic species richness without plants and fungi_protists
apply(merged_diversity3[,c('mam_tot','bir_tot','rep_tot','amp_tot')], 1, sum, na.rm=TRUE)
merged_diversity3$endemSRtot <- apply(merged_diversity3[,c('mam_tot','bir_tot','rep_tot','amp_tot')], 1, sum, na.rm=TRUE)

## remove 0s
merged_diversity3 <- merged_diversity3[-which(merged_diversity3$endemSRtot==0),]
merged_diversity3

# Scatterplot with regression line
Fig_S1c <- ggplot(merged_diversity3, aes(x = endemSRtot, y = endem, label=cntry.code)) +
  geom_point(aes(color=continentcountry), size=5) +                     # points
  scale_x_log10() +
  scale_y_log10() +
  geom_smooth(method = "lm", se = TRUE, color = "darkred") +
  ...
  #geom_text_repel(size=3)+# regression line with CI
  labs(
    x = "richness of endemic animal species",
    y = "endemic taxonomic richness in sport emblems"
  ) +
  theme_classic()+
  theme(
...
## scale endemSRtot
merged_diversity3$lendemSRtot.sc <- scale(log10(merged_diversity3$endemSRtot), scale=T, center=T)


## run a simple linear regression
model_div_3.1 <- lm(lendem.sc ~ lendemSRtot.sc + lnwildteams.sc, data = merged_diversity3)
summary(model_div_3.1)
check_model(model_div_3.1)
plot_model(model_div_3.1)
model_div_3.2 <- lm(lendem.sc ~ lendemSRtot.sc, data = merged_diversity3)
model_div_3.3 <- lm(lendem.sc ~ lnwildteams.sc, data = merged_diversity3)
model_div_3.4 <- lm(lendem.sc ~ 1, data = merged_diversity3)

LL.vals <- c(as.numeric(logLik(model_div_3.1)), as.numeric(logLik(model_div_3.2)), 
             as.numeric(logLik(model_div_3.3)), as.numeric(logLik(model_div_3.4)))
AICc.vals <- AICc(model_div_3.1, model_div_3.2, model_div_3.3, model_div_3.4)
delta.AICc.vals <- delta.AIC(AICc.vals)
weight.AICc.vals <- weight.AIC(delta.AICc.vals)
R2adj.vals <- c(summary(model_div_3.1)$adj.r.squared, summary(model_div_3.2)$adj.r.squared, 
                summary(model_div_3.3)$adj.r.squared, summary(model_div_3.4)$adj.r.squared)
mod.labs <- c("full","richness","teams","intercept")
k.vals <- c(3,2,2,1)

# summary table
model.summary.3 <- data.frame(model=mod.labs, k=k.vals, LL=round(LL.vals,3), AICc=round(AICc.vals,3), 
                              ΔAICc=round(delta.AICc.vals,3), wAICc=round(weight.AICc.vals,3),
                              R2adj=round(R2adj.vals,3))
# sort descending on wAICc
model.summary2.3 <- model.summary.3[order(-model.summary.3$wAICc),]
model.summary2.3

# Scatterplot with regression line
Fig_S1d <- ggplot(merged_diversity3, aes(x = endemSRtot, y = nsp4, label=cntry.code)) +
  geom_point(aes(color=continentcountry), size=5) +                     # points
  scale_x_log10() +
  scale_y_log10() +
  geom_smooth(method = "lm", se = TRUE, color = "darkred") +
...
## run a simple linear regression
model_div_4.1 <- lm(lnsp4.sc ~ lendemSRtot.sc + lnwildteams.sc, data = merged_diversity3)
summary(model_div_4.1)
check_model(model_div_4.1)
plot_model(model_div_4.1)
model_div_4.2 <- lm(lnsp4.sc ~ lendemSRtot.sc, data = merged_diversity3)
model_div_4.3 <- lm(lnsp4.sc ~ lnwildteams.sc, data = merged_diversity3)
model_div_4.4 <- lm(lnsp4.sc ~ 1, data = merged_diversity3)

LL.vals <- c(as.numeric(logLik(model_div_4.1)), as.numeric(logLik(model_div_4.2)), 
             as.numeric(logLik(model_div_4.3)), as.numeric(logLik(model_div_4.4)))
AICc.vals <- AICc(model_div_4.1, model_div_4.2, model_div_4.3, model_div_4.4)
delta.AICc.vals <- delta.AIC(AICc.vals)
weight.AICc.vals <- weight.AIC(delta.AICc.vals)
R2adj.vals <- c(summary(model_div_4.1)$adj.r.squared, summary(model_div_4.2)$adj.r.squared, 
                summary(model_div_4.3)$adj.r.squared, summary(model_div_4.4)$adj.r.squared)
mod.labs <- c("full","richness","teams","intercept")
k.vals <- c(3,2,2,1)

# summary table
model.summary.4 <- data.frame(model=mod.labs, k=k.vals, LL=round(LL.vals,3), AICc=round(AICc.vals,3), 
                              ΔAICc=round(delta.AICc.vals,3), wAICc=round(weight.AICc.vals,3),
                              R2adj=round(R2adj.vals,3))
# sort descending on wAICc
model.summary2.4 <- model.summary.4[order(-model.summary.4$wAICc),]
model.summary2.4

## export versions of merged_diversity
getwd()
#write.csv(merged_diversity, "merged_diversity.csv")
#write.csv(merged_diversity2, "merged_diversity2.csv")
#write.csv(merged_diversity3, "merged_diversity3.csv")



##############################################
##############################################
## 8. PREPARATION OF FIGURE S3 (=sports biplot)
##############################################
##############################################
nsp=c()
...
Fig_S5 <- ggplot(datsports, aes(x=nsp, y=relative,label=Sport)) + 
  geom_point(aes(color=Sport), size=12)+
  geom_text_repel(size=3)+
  theme_classic()+
  theme(axis.text=element_text(size=15))+
...
##################################################################################################################
##################################################################################################################
## 9. PREPARATION OF FIGURE 5 (=presence/absence of species for each country)
## Focusing on the 84 species (86 species - 2 species absent from the IUCN database)
##################################################################################################################
##################################################################################################################
datpresence <- subset(dat,Classification_General=="Species")
unique(datpresence$Classification_Specific) # the list of 86 taxa identified to species level
length(which(datpresence$N_Species_In_Country==0)) # 229 instances of species not present in the country // 227 post-correction
length(which(datpresence$N_Species_In_Country==1)) # 214 instances of species present in the country //215 post-correction
length(which(is.na(datpresence$N_Species_In_Country)==TRUE)) # 2 NAs (absent from the IUCN database)

# Calculating the  number of times a species is present in the country
pres=c()
abs=c()
Nspecies=c()
continentcountry=c()
...
datpresabs$continentcountry <-  factor(datpresabs$continentcountry, levels=c("Europe", "Americas", "Oceania", "Africa", "Asia"))
...
Fig5 <- ggplot(long_df, aes(x = country, y = Value, fill = Presabs)) + 
...
##################################################################################################################################
##################################################################################################################################
## 10. PREPARATION OF FIGURE 3 (=conservation status)

## Focusing on the 84 species (86 species - 2 species absent from the IUCN database)
##################################################################################################################################
##################################################################################################################################
datconserv <- subset(dat,Classification_General=="Species")
unique(datconserv$Classification_Specific)
...
# "Latrodectus hasselti" "Myrmecia pilosula"

# plotting the representation of classes among the 84 species
...
##############################################
##############################################
## 11. ISOLATING DATA FOR STATISTICAL ANALYSIS
##############################################
##############################################
...
# Preparing Figure S2
Fig_S2 <- ggplot(mod6.outtab, aes(x = var, y = est)) +
